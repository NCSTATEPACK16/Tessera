...
// Bumped to 2 at step 6 for the `daily` store, and to 3 at step 8 for the
// `completions` store. The upgrade below guards every `createObjectStore` with
// a `contains` check, so a bump is purely additive — an existing player's
// sessions, photos, thumbnails and daily survive it. Getting that wrong deletes
// every in-progress puzzle a real player has, which is why `persistence.spec.ts`
// asserts the v2→v3 bump directly rather than trusting this comment.
const DB_VERSION = 3;
