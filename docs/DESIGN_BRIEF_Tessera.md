...
So the two things this app must beat everyone at:
...
2. **The snap is the product.** Layered audio, haptics, a piece that overshoots by two pixels and settles, neighbours that ripple outward. Budget real time for this.
...
### Signature element
**The photo colours the app.** On load, extract the 3 dominant colours from the source photo and drive the session's accent, the bloom hue, and the tray highlight from them. A photo of a beach gives you a warm sand-and-teal session; a night city gives you cyan and amber. The chrome is neutral by design so the photo owns the palette. Every puzzle looks like a different app in the best way.
...
- **Snap** is the exception: piece travels to slot in ~90ms, overshoots 2-3% of piece size, settles in 140ms with a slight scale bounce. Neighbouring placed pieces get a 1px outward ripple, staggered 20ms from the contact point.
...
5. **Board — play.** The centrepiece. Needs: mat, camera (pinch/pan) with zoom indicator, the tray, progress ring, timer (dismissible), hint button, pause. *States: default, zoomed in on a region, tray expanded to half-sheet, tray sorted by colour, one-handed iPhone reach layout, iPad landscape layout with tray docked right.*
...
| **Rush** | 3 min, score by pieces placed, endless piece feed | The 5-minute-queue mode |
